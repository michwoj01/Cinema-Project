...
public class SelectMovieController {
    ...
    @FXML
    private TableColumn<Schedule, Schedule> movieDate;
    @FXML
    private TableColumn<Schedule, Schedule> movieCinemaHall;

    @FXML
    private TableColumn<Schedule,Schedule> movieAvailableSeats;
    ...
    public static URL getFXML() {
        return ModifyScheduleWatchersController.class.getResource("/fxml/SelectMoviePage.fxml");
    }
    ...
    public void initialize() {
        ...
        movieCinemaHall.setCellValueFactory(new PropertyValueFactory<>("cinema_hall_id"));
        ...
        scheduleMovieTableView.getSelectionModel().selectedItemProperty().addListener(((observable, oldValue, newValue) -> {
            if (newValue != null){
                onSelectedSchedule(observable.getValue());
            }
        }));
        scheduleService.getAll().toStream().forEach(schedule->scheduleMovieTableView.getItems().add(schedule));
    }
    public void onSelectedSchedule(Schedule newSchedule) {
        SceneChanger.setPane(MovieCardController.getFXML());
        applicationContext.publishEvent(new ScheduleEvent(newSchedule));
    }
